...
const Updatehorario = () => {
  ...
  useEffect(() => {
    fetchdata(endpoint, options) // Fetch staff list when component mounts
  }, [fetchdata])

  // Handle staff data after fetching
  useEffect(() => {
  ...
  // Fetch all horarios for selected staff
  const handleSelectStaff = async (staffId) => {
    ...
    setUpdatedHorario({})
    setHorarioList([]) // Clear previous horarios before fetching new ones
